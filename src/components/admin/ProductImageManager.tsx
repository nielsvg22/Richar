...
export default function ProductImageManager({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/admin/product-images?productSlug=${encodeURIComponent(slug)}`)
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/product-images?productSlug=${encodeURIComponent(slug)}`);
  ...
  return (
    <div className="rounded-2xl border border-ink/10 bg-white p-4">
      <p className="mb-3 text-xs text-ink-soft">De eerste foto is de hoofdafbeelding.</p>
      {loading ? (
