...
export default function WorkshopImageManager({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/admin/workshop-images?workshopSlug=${encodeURIComponent(slug)}`)
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/workshop-images?workshopSlug=${encodeURIComponent(slug)}`);
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      formData.append("workshopSlug", slug);
      formData.append("file", file);
      const res = await fetch("/api/admin/workshop-images", { method: "POST", body: formData });
      if (!res.ok) {
  ...
  async function handleDelete(id: string) {
    ...
    const res = await fetch(`/api/admin/workshop-images/${id}`, { method: "DELETE" });
  ...
  return (
    <div className="rounded-2xl border border-ink/10 bg-white p-4">
      <p className="mb-3 text-xs text-ink-soft">
        De eerste foto is de hoofdafbeelding op de workshoppagina en kaarten.
      </p>
      {loading ? (
        ...
        <div className="grid grid-cols-3 gap-3 sm:grid-cols-4">
          {images.map((img, i) => (
            <div key={img.id} className="group relative aspect-square overflow-hidden rounded-xl">
              ...
              />
              {i === 0 && (
                <span className="absolute left-1 top-1 rounded-full bg-ink/70 px-2 py-0.5 text-[10px] font-semibold text-cream">
                  Hoofd
                </span>
              )}
              <button
